...
import { ArrowRight, BarChart3, Database, AlertCircle, BookOpen, Code } from 'lucide-react'
import Header from '@/components/Header'
import Footer from '@/components/Footer'
import ExpandableStoryCard from '@/components/ExpandableStoryCard'
import type { Stats, USATrends, MultiCountryData } from '@/lib/api'
...
// Direct imports for chart components (no dynamic() - data is pre-fetched)
import USATrustTimeline from '@/components/charts/USATrustTimeline'
import RuleLawTrends from '@/components/charts/RuleLawTrends'
import CovidTrustImpact from '@/components/charts/CovidTrustImpact'
import TrustCollapse from '@/components/charts/TrustCollapse'
import TrustInversion from '@/components/charts/TrustInversion'
import FinancialTrustParadox from '@/components/charts/FinancialTrustParadox'
...
// GitHub mark — lucide-react 1.x removed brand icons (trademark). Inlined here.
...
export interface HomePageChartData {
...
export interface HomePageClientProps {
  stats: Stats | null
  chartData: HomePageChartData
  heroIndices: [number, number]
}

// Story configuration with pre-bound data
...
export default function HomePageClient({ stats, chartData, heroIndices }: HomePageClientProps) {
  const [expandedStoryId, setExpandedStoryId] = useState<string | null>(null)

  // Check URL hash to auto-expand matching story (client-side only)
  useEffect(() => {
  ...
  // Build stories with pre-fetched data
  const STORIES: StoryConfig[] = [
    {
      ...
      renderChart: () => <USATrustTimeline initialData={chartData.usaTrends} />,
    },
    {
      id: 'rule-of-law',
      ...
      renderChart: () => <RuleLawTrends initialData={chartData.ruleLawTrends} />,
    },
    {
      id: 'covid-impact',
      ...
      renderChart: () => <CovidTrustImpact initialData={chartData.covidImpact} />,
    },
    {
      id: 'trust-collapse',
      ...
      renderChart: () => <TrustCollapse initialData={chartData.trustCollapse} />,
    },
    {
      id: 'trust-inversion',
      ...
      renderChart: () => <TrustInversion initialData={chartData.trustInversion} />,
    },
    {
      ...
      renderChart: () => <FinancialTrustParadox initialData={chartData.financialParadox} />,
    },
  ]

  // Select heroes and cards based on server-computed indices
  const heroSet = new Set(heroIndices)
  const heroStories = STORIES.filter((_, i) => heroSet.has(i))
  const cardStories = STORIES.filter((_, i) => !heroSet.has(i))
  ...
  return (
    <div className="min-h-screen">
      ...
      <main>
        ...
        {/* Hero Stories (2 selected by week) */}
